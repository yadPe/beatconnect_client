...
const binary = path.join(__dirname, process.env.NODE_ENV === 'production' ? 'build' : 'public', 'macos-wallpaper');
...
exports.set = async (imagePath, { screen = 'all', scale = 'auto' } = {}) => {
  ...
  const arguments_ = ['set', path.resolve(imagePath), '--screen', screen, '--scale', scale];

...
const getDlLink = (beatmapInfos, pretty, extra) => {
    ...
    if (extra){
        ...
        const { creator, approved, version, creator_id, bpm, max_combo, difficultyrating, diff_approach } = extra;
        return `${status[approved]} | [https://beatconnect.io/b/${id}/${unique_id} ${artist} - ${title} - ${version}] by [https://osu.ppy.sh/u/${creator_id} ${creator}] | BPM: ${bpm} | AR:${diff_approach} | Max combo: ${max_combo}`;
    }

...
class Bot {
  ...
  onMessage(from, channel, text, rawMsg){
    console.log(rawMsg);
    if(rawMsg.nick === "BanchoBot"){
      this.irc.banchoMsg(text)
    }
    ...
    switch(command){
      ...
      case 'infos' :
        ...
        break;
      case 'createRoom':
        this.irc.makeMatch(params[0]).then(response => this.irc.pm(`Your`))
        break;
      case 'search':
        this.beatconnect.searchBeatmap(params).then(result => this.irc.pm(from, result));
        break;
      default:

...
class OsuIrc {
  constructor(onMessage) {
    ...
    this.client.addListener('raw', msg => msg.command !== 'QUIT' ? console.log(msg) : null)
  }

  banchoMsg(message) {
    eventEmitter.emit('banchoMsg', { message });
  }
  ...
  makeMatch(name) {
    return new Promise((resolve, reject) => {
      ...
      // const timeout = setTimeout(() => { eventEmitter.removeAllListeners(); reject('Timed out'); }, 5000);
      // eventEmitter.on('banchoMsg', msg => {
      //   console.log(msg)
      //   if (msg.toLowerCase().includes('created')) {
      //     clearTimeout(timeout);
      //     eventEmitter.removeAllListeners();
      //     //const matchId = clearTimeout 
      //     resolve(msg)
      //   }
      // });
    });
